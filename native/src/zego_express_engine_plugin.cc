...
class ZegoExpressBridge : public std::enable_shared_from_this<ZegoExpressBridge>,
  ...
  public:
    static std::string getVersion() { return ZegoExpressSDK::getVersion(); }
  ...
  public:
    void onDebugError(int errorCode, const std::string &funcName,
  ...
  public:
    ...
    void onRemoteVideoFrameRawData(unsigned char **data, unsigned int *dataLength,
        ...
        printf(
            "[onRemoteVideoFrameRawData] streamID:%s, data:%p, length:%u, w:%d, h:%d, format:%d\n",
